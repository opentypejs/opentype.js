...
"use strict";function parseGlyphCoordinate(r,e,t,o,n){var a;return(e&o)>0?(a=r.parseByte(),0===(e&n)&&(a=-a),a=t+a):a=(e&n)>0?t:t+r.parseShort(),a}function parseGlyph(r,e,t){var o=new parse.Parser(e,t);r.numberOfContours=o.parseShort(),r.xMin=o.parseShort(),r.yMin=o.parseShort(),r.xMax=o.parseShort(),r.yMax=o.parseShort();var n,a;if(r.numberOfContours>0){var s,p=r.endPointIndices=[];for(s=0;s<r.numberOfContours;s+=1)p.push(o.parseUShort());for(r.instructionLength=o.parseUShort(),r.instructions=[],s=0;s<r.instructionLength;s+=1)r.instructions.push(o.parseByte());var u=p[p.length-1]+1;for(n=[],s=0;u>s;s+=1)if(a=o.parseByte(),n.push(a),(8&a)>0)for(var i=o.parseByte(),l=0;i>l;l+=1)n.push(a),s+=1;if(check.argument(n.length===u,"Bad flags."),p.length>0){var h,y=[];if(u>0){for(s=0;u>s;s+=1)a=n[s],h={},h.onCurve=!!(1&a),h.lastPointOfContour=p.indexOf(s)>=0,y.push(h);var c=0;for(s=0;u>s;s+=1)a=n[s],h=y[s],h.x=parseGlyphCoordinate(o,a,c,2,16),c=h.x;var f=0;for(s=0;u>s;s+=1)a=n[s],h=y[s],h.y=parseGlyphCoordinate(o,a,f,4,32),f=h.y}r.points=y}else r.points=[]}else if(0===r.numberOfContours)r.points=[];else{r.isComposite=!0,r.points=[],r.components=[];for(var v=!0;v;){n=o.parseUShort();var x={glyphIndex:o.parseUShort(),xScale:1,scale01:0,scale10:0,yScale:1,dx:0,dy:0};(1&n)>0?(x.dx=o.parseShort(),x.dy=o.parseShort()):(x.dx=o.parseChar(),x.dy=o.parseChar()),(8&n)>0?x.xScale=x.yScale=o.parseF2Dot14():(64&n)>0?(x.xScale=o.parseF2Dot14(),x.yScale=o.parseF2Dot14()):(128&n)>0&&(x.xScale=o.parseF2Dot14(),x.scale01=o.parseF2Dot14(),x.scale10=o.parseF2Dot14(),x.yScale=o.parseF2Dot14()),r.components.push(x),v=!!(32&n)}}}function transformPoints(r,e){for(var t=[],o=0;o<r.length;o+=1){var n=r[o],a={x:e.xScale*n.x+e.scale01*n.y+e.dx,y:e.scale10*n.x+e.yScale*n.y+e.dy,onCurve:n.onCurve,lastPointOfContour:n.lastPointOfContour};t.push(a)}return t}function getContours(r){for(var e=[],t=[],o=0;o<r.length;o+=1){var n=r[o];t.push(n),n.lastPointOfContour&&(e.push(t),t=[])}return check.argument(0===t.length,"There are still points left in the current contour."),e}function getPath(r){var e=new path.Path;if(!r)return e;for(var t=getContours(r),o=0;o<t.length;o+=1){var n,a,s=t[o],p=s[0],u=s[s.length-1];p.onCurve?(n=null,a=!0):(p=u.onCurve?u:{x:(p.x+u.x)/2,y:(p.y+u.y)/2},n=p,a=!1),e.moveTo(p.x,p.y);for(var i=a?1:0;i<s.length;i+=1){var l=s[i],h=0===i?p:s[i-1];if(h.onCurve&&l.onCurve)e.lineTo(l.x,l.y);else if(h.onCurve&&!l.onCurve)n=l;else if(h.onCurve||l.onCurve){if(h.onCurve||!l.onCurve)throw new Error("Invalid state.");e.quadraticCurveTo(n.x,n.y,l.x,l.y),n=null}else{var y={x:(h.x+l.x)/2,y:(h.y+l.y)/2};e.quadraticCurveTo(h.x,h.y,y.x,y.y),n=l}}p!==u&&(n?e.quadraticCurveTo(n.x,n.y,p.x,p.y):e.lineTo(p.x,p.y))}return e.closePath(),e}function buildPath(r,e){if(e.isComposite)for(var t=0;t<e.components.length;t+=1){var o=e.components[t],n=r.get(o.glyphIndex);if(n.points){var a=transformPoints(n.points,o);e.points=e.points.concat(a)}}return getPath(e.points)}function parseGlyfTable(r,e,t,o){var n,a=new _glyphset(o);for(n=0;n<t.length-1;n+=1){var s=t[n],p=t[n+1];s!==p?a.push(n,_glyphset.ttfGlyphLoader(o,n,parseGlyph,r,e+s,buildPath)):a.push(n,_glyphset.glyphLoader(o,n))}return a}var check=require("../check"),_glyphset=require("../glyphset"),parse=require("../parse"),path=require("../path");exports.parse=parseGlyfTable;
